...
function addUserMarker() {
    const userMarker = new google.maps.Marker({
        ...
        title: "당신의 위치",
        icon: {
            url: "https://maps.google.com/mapfiles/ms/icons/black-dot.png", // 검정색 마커 URL
            scaledSize: new google.maps.Size(32, 32),  // 마커 크기 조정
            anchor: new google.maps.Point(16, 32)  // 마커 위치 조정
        }
    });
}
...
function generateRoute() {
    ...
    nearbyLocations.forEach(location => {
        const marker = new google.maps.Marker({
            ...
            icon: markerColors[location.locType]  // 마커 색상
        });
        markers.push(marker);
    });
}
...
function initMap() {
    ...
    // 사용자 위치 가져오기
    getUserLocation();  // 위치 정보와 마커를 초기화하는 함수 호출
}
